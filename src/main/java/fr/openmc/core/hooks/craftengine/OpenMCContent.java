...
public final class OpenMCContent {
    ...
    public static @Nullable BukkitFurniture furnitureAt(Location location) {
        if (location.getWorld() == null) return null;
        return location.getWorld().getNearbyEntities(location.clone().add(0.5, 0.5, 0.5), 0.75, 0.75, 0.75)
                .stream()
                .map(OpenMCContent::furniture)
                .filter(java.util.Objects::nonNull)
                .findFirst()
                .orElse(null);
    }

    public static @Nullable BukkitFurniture placeFurniture(Location location, String id) {
        return CraftEngineFurniture.place(location, Key.of(id));
    }

    public static boolean removeFurniture(BukkitFurniture furniture) {
        return furniture != null && CraftEngineFurniture.remove(furniture.bukkitEntity(), false, false);
    }
